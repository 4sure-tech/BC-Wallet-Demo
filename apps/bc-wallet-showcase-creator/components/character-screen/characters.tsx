    ...
    <>
      <div className="flex w-full h-screen overflow-hidden">
        <div className="flex gap-4 p-4 w-full">
          ...
          <div className="w-1/3 bg-white dark:bg-dark-bg-secondary  shadow-md rounded-md flex flex-col">
            <div className="p-4">
              <h2 className="text-md font-bold">{t('character.create_your_character_title')}</h2>
              <p className="text-xs">{t('character.create_your_character_subtitle')}</p>
            </div>
                    ...
                    <div
                      ...
                      className={cn(
                        ...
                        selectedPersonaId === persona.id
                          ? 'flex flex-col items-center  bg-foreground/10 '
                          : 'flex flex-row items-center border-t border-b border-foreground/10 bg-foreground/5 hover:bg-light-bg dark:hover:bg-dark-input-hover'
                      )}
                      onClick={() => handlePersonaSelect(persona)}
                    >

                      {selectedPersonaId === persona.id && persona.hidden && (
                        <div className="flex gap-2 items-center absolute top-4 left-24 bg-foreground text-foreground px-4 py-2 text-sm font-medium rounded">
                          <EyeOff size={22} />
                          {t('character.hidden_label')}
                        </div>
                      )}

                      <div className={`shrink-0 ${selectedPersonaId === persona.id ? 'my-4' : ''}`}>
                        <Image
                          src={
                      ...
                      <div className={cn(
                        selectedPersonaId === persona.id
                          ? 'text-center w-full break-words px-4' // enforce wrapping
                          : 'flex-1 px-4'
                      )}>
                        <h3 className="text-lg text-foreground font-semibold break-words">{persona.name}</h3>
                        <p className="text-sm text-foreground/80">{persona.role}</p>
                        {selectedPersonaId === persona.id && (
                          <p className="text-xs text-foreground/60 mt-2 whitespace-pre-wrap break-words">
                            {persona.description}
                          </p>
                        )}
                      </div>
          ...
          {/* Right panel - Form */}
          <div className="w-2/3 h-full bg-white dark:bg-dark-bg-secondary border shadow-md rounded-md p-6 flex flex-col overflow-y-auto">
                  ...
                  <form onSubmit={form.handleSubmit(handleFormSubmit)}>
                    <div>
                      ...
                      <div className="mt-auto pt-4 border-t flex justify-end gap-3">
                        <Button variant="outlineAction" onClick={handleCancel}>{t('action.cancel_label')}</Button>
                        <Button variant="outlineAction" type="submit" disabled={!form.formState.isValid}>
                          {t('action.save_label')}
                        </Button>
                        <Button variant="outlineAction" onClick={(e) => {
                          e.preventDefault()
                          handleProceed()
